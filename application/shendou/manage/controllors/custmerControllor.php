...
class custmerControllor extends Shendou_Controllor_ManageAbstract
{
	...
	public function configAction()
	{
		...
		if ($this->mRequest->isPost()) {
			...
			$webInfo['webInfo']['site']['mode'] = isset($webInfo['webInfo']['site']['mode']) ? 2 : 1;
			$webInfo['webInfo']['site']['isSend'] = isset($webInfo['webInfo']['site']['isSend']) ? 1 : 0;
			$webInfo['webInfo']['upload']['isWatermark'] = isset($webInfo['webInfo']['upload']['isWatermark']) ? 1 : 0 ;
			$webInfo['webInfo']['upload']['allowFiles']  = explode("\r\n", $webInfo['webInfo']['upload']['allowFiles']);
			
			$webInfo = "<?php\nreturn " . var_export($webInfo, true) . ';';
			
			Lamb_IO_File::putContents(DATA_PATH . 'config.var.php', $webInfo);
